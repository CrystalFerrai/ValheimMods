Adds configuration options to modify comfort item radius, fire warmth radius, rested buff base time and rested buff time per comfort level.
...
2. Download latest ``Comfortable.dll`` by clicking "Manual Download". Extract the dll from the zip file into ``[GameDirectory]\Bepinex\plugins``. (You only need the dll.)
