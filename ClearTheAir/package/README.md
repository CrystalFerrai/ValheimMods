Add a configuration option to modify the effective radius of Mistlands fog clearing items such as Wisp Torches and Wisplights.
...
3. Download latest ``ClearTheAir.dll`` by clicking "Manual Download". Extract the dll from the zip file into ``[GameDirectory]\Bepinex\plugins``. (You only need the dll.)
